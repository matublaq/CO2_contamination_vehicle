...
st.markdown("<p style='font-size: 12px;'>No tendremos en cuenta los vehiculos electricos o hibridos</p>", unsafe_allow_html=True)
...
st.markdown("<p style='font-size: 25px;'>Datos generales</p>", unsafe_allow_html=True)
...
def generate_pairplot():
    sns.pairplot(df)
    st.pyplot(plt.show())
st.write('Este grafico requiero de 2 minutos aproximadamente')
if st.button('Relacion entre las variables'):
    generate_pairplot()
st.markdown('<br>', unsafe_allow_html=True)
...
#####################################################################
#Análisis
st.markdown("<p style='font-size: 20px;'>Podemos destacar que</p>", unsafe_allow_html=True)
st.markdown("\n- Mayor consumo de combustible, mayor seran las emisiones de CO2. \n- Menor peso, menor va a ser el consumo.", unsafe_allow_html=True)
